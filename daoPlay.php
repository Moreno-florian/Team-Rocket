...
class DaoPlay
{
    ...
    public function modifyPlay($play): void
    {
        ...
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("UPDATE jouer SET Id_joueur=?,Id_match=?,Id_equipe_adverse=?,But_marque_par_match=?,Passe_decisive_par_match=?,Poste=?,Temps_joue_par_match=? WHERE Id_match = ?"); // Prepare the request to be execute

        try {
            $request->execute(array($play->get_idPlayer(), $play->get_idMatch(), $play->get_idRivalTeam(), $play->get_goalByMatch(), $play->get_assist(), $play->get_position(), $play->get_playTimeByMatch(), $play->get_idMatch()));    // Execute the request
        } catch (Exception $e) {
            echo ('Erreur : ' . $e->getMessage() . ' ! ');    // Send the error message.		
        }
    }
    ...
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("DELETE FROM jouer WHERE Id_match =?"); // Prepare the request to be execute
    ...
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("DELETE FROM jouer WHERE Id_Player =? "); // Prepare the request to be execute

        try {
            $request->execute(array($idPlayer));    // Execute the request
        } catch (Exception $e) {
            echo ('Erreur : ' . $e->getMessage() . ' ! ');    // Send the error message.		
        }
    }
    ...
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("DELETE FROM jouer WHERE Id_equipe_adverse = ?"); // Prepare the request to be execute

        try {
            $request->execute(array($idRvalTaem));    // Execute the request
        } catch (Exception $e) {
            echo ('Erreur : ' . $e->getMessage() . ' ! ');    // Send the error message.		
        }
    }
    ...
    public function addPlay($play): void    // Used for match creation too 
    {
        ...
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("INSERT INTO jouer (Id_joueur,Id_match,Id_equipe_adverse,But_marque_par_match,Passe_decisive_par_match,Poste,Temps_joue_par_match) VALUES (?,?,?,?,?,?,?)"); // Prepare the request to be execute


        try {
            $request->execute(array($play->get_idPlayer(), $play->get_idMatch(), $play->get_idRivalTeam(), $play->get_goalByMatch(), $play->get_assist(), $play->get_position(), $play->get_playTimeByMatch()));    // Execute the request
        } catch (Exception $e) {
            echo ('Erreur : ' . $e->getMessage() . ' ! ');    // Send the error message.		
        }
    }
    ...
    public function showOnePlayerRegistration($idPlayer, $idMatch): array
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $tab = [];
        $request = $pdo->prepare("SELECT * FROM jouer WHERE Id_joueur= ? AND Id_MAtch=?"); // Prepare the request to be execute

        try {
            $request->execute(array($idPlayer, $idMatch));    // Execute the request
    ...
    public function modifyPlayerRegistration($play): void
    {
        ...
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("UPDATE jouer SET Id_joueur=?,Id_match=?,Id_equipe_adverse=?,But_marque_par_match=?,Passe_decisive_par_match=?,Poste=?,Temps_joue_par_match=? WHERE Id_match =? AND Id_joueur =?"); // Prepare the request to be execute

        try {
            $request->execute(array($play->get_idPlayer(), $play->get_idMatch(), $play->get_idRivalTeam(), $play->get_goalByMatch(), $play->get_assist(), $play->get_position(), $play->get_playTimeByMatch(), $play->get_idMatch(), $play->get_idPlayer())); // Prepare the request to be execute));    // Execute the request
        } catch (Exception $e) {
            echo ('Erreur : ' . $e->getMessage() . ' ! ');    // Send the error message.		
        }
    }
