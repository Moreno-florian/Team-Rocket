...
class DaoPlayer
{
    public function addPlayer($player): void
    {
        ...
        $request = $pdo->prepare("INSERT INTO joueurs (Nom,Prenom,Annee_d_arrivee,Poste_principal,Photo,Numero_de_licence,Id_utilisateur) VALUES (?,?,?,?,?,?,?)");     // Prepare the request to be execute
        try {
            $request->execute(array($player->get_lastname(), $player->get_firstname(), $player->get_arrivalYear(), $player->get_position(), $player->get_picture(), $player->get_licenseNumber(), $player->get_idUser()));    // Execute the request
    ...
    public function modifyPlayer($player): void
    {
        ...
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("UPDATE joueurs SET Nom=?,Prenom=?,Annee_d_arrivee=?,Poste_principal=?,Photo=?,Numero_de_licence=? WHERE Id_joueur=?");     // Prepare the request to be execute

        try {
            $request->execute(array($player->get_lastname(), $player->get_firstname(), $player->get_arrivalYear(), $player->get_position(), $player->get_picture(), $player->get_licenseNumber(), $player->get_idPlayer()));    // Execute the request
    ...
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("DELETE FROM joueurs WHERE joueurs.Id_joueur = ?"); // Prepare the request to be execute
