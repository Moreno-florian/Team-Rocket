...
class DaoPicture
{
    ...
    public function showOnePicture($idPicture): array
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $tab = [];
        $request = $pdo->prepare("SELECT * FROM photo WHERE Id_photo= ?"); // Prepare the request to be execute
    ...
    public function addPicture($picture): int
    {
        ...
        try {
            $request->execute(array($picture->get_title(), $picture->get_urlPicture()));    // Execute the request
    ...
    public function modifyPicture($picture): void
    {
        ...
        $request = $pdo->prepare("UPDATE photo SET Titre=?,Url_photo=? WHERE Id_photo=?");

        try {
            $request->execute(array($picture->get_title(), $picture->get_urlPicture(), $picture->get_idPicture()));    // Execute the request
    ...
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("DELETE FROM photo WHERE Id_photo=?"); // Prepare the request to be execute
