...
class DaoUser
{
    ...
    public function addUser($User): int
    {
        ...
        $request = $pdo->prepare("INSERT INTO utilisateur (Pseudo,Password,Mail,Role) VALUES (?,?,?,?)"); // Prepare the request to be execute

        try {
            $request->execute(array($User->get_Pseudo(), $passwordHash, $User->get_mail(), $User->get_role()));    // Execute the request
    ...
    public function modifyUser($User): void
    {
        ...
        $pdo = Dao::getConnection();     // Connection to the DB
        $passwordHash = password_hash($User->get_password(),  PASSWORD_DEFAULT);    // Create a hash key for a password
        $request = $pdo->prepare("UPDATE utilisateur SET Pseudo=?,Password=?,Mail=?,Role=? WHERE Id_utilisateur=?"); // Prepare the request to be execute

        try {
            $request->execute(array($User->get_Pseudo(), $passwordHash, $User->get_mail(), $User->get_role(), $User->get_idUser()));    // Execute the request
        } catch (Exception $e) {
            echo ('Erreur : ' . $e->getMessage() . ' ! ');    // Send the error message.		
        }
    }
    ...
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("DELETE FROM utilisateur WHERE Id_utilisateur=?"); // Prepare the request to be execute

        try {
            $request->execute(array($idUser));    // Execute the request
        } catch (Exception $e) {
            echo ('Erreur : ' . $e->getMessage() . ' ! ');    // Send the error message.		
        }
    }
